...
public class Conjunction {
	...
	/**
	 *  Almost very sophisticated search: with unit propagation, 
	 *  pure variable elimination, but no backjumping,
	 *  and a smart strategy for picking decision variables. 
	 */
	public static SearchControl almostSuperFancySearch = new SearchControl() {
	    /* TBC: Your code here, replacing the line below */
		public void simplify(Conjunction c) {
			c.propagateUnitClauses();
			c.assumePureLiterals();
	...
	/**
	 *  Not too sophisticated search: with unit propagation, 
	 *  pure variable elimination, no backjumping,
	 *  and a no smart strategy for picking decision variables. 
	 */
	public static SearchControl onlyUPAndPLSearch = new SearchControl() {
	    /* TBC: Your code here, replacing the line below */
		public void simplify(Conjunction c) {
			c.propagateUnitClauses();
			c.assumePureLiterals();
	...
	/**
	 *  Not a very sophisticated search: no unit propagation, 
	 *  no pure variable elimination, backjumping,
	 *  and no smart strategy for picking decision variables. 
	 */
	public static SearchControl backJumpSearch = new SearchControl() {
	    /* TBC: Your code here, replacing the line below */
		public void simplify(Conjunction c) {;}
		public Assignment pick(Conjunction c) { 
			return new Assignment(c.alphabeticallyFirstVariable(), new Boolean(true)); }
	 ...
	 *  and a smart strategy for picking decision variables. 
	 */
	public static SearchControl backJumpAndStratSearch = new SearchControl() {
	    /* TBC: Your code here, replacing the line below */
		public void simplify(Conjunction c) {;}
		public Assignment pick(Conjunction c) { 
			return new Assignment(c.mostFrequentVariable().v, new Boolean(true)); }
	...
	/**
	 *  Really, really not a very sophisticated search: no unit propagation, 
	 *  no pure variable elimination, no backjumping,
	 *  but a smart strategy for picking decision variables. 
	 */
	public static SearchControl poorMansSearch = new SearchControl() {
	    /* TBC: Your code here, replacing the line below */
		public void simplify(Conjunction c) {;}
		public Assignment pick(Conjunction c) { 
			return new Assignment(c.mostFrequentVariable().v, new Boolean(true)); }
	 ...
	 */
	/*int satisfiable(SearchControl control, SearchStats stats, int depth) {
		this.control.simplify(this);
		if (this.)
	...
	SearchResult search(SearchControl control, SearchStats stats, int depth) {
		...
		if (this.isConsistent()) {
			SearchResult result = new SearchResult(this.assignments);
			stats.succeed(depth);
			return result;
		}
		stats.expand(depth);
		Assignment next = control.pick(this);
		SearchResult result;
		SearchResult result2;
		if ((result = searchHelper(new Conjunction(this, next.v, next.b), control, stats, (depth + 1))).status == SearchStatus.Success) {
			return result;
		}
		if (result.status == SearchStatus.Timeout) {
			return result;
		}
		if ((result2 = searchHelper(new Conjunction(this, next.v, !next.b), control, stats, (depth + 1))).status == SearchStatus.Success) {
			return result2;
		} 
		if (result2.status == SearchStatus.Timeout) {
			return result2;
		} else {
	 ...
	 * @return search result structure describing what happened
	 */
	SearchResult searchHelper(Conjunction c, SearchControl control, SearchStats stats, int depth) {
		...
		if (c.isConsistent()) {
			SearchResult result = new SearchResult(c.assignments);
			stats.succeed(depth);
			return result;
		}
		stats.expand(depth);
		Assignment next = control.pick(c);
		SearchResult result;
		SearchResult result2;
		if ((result = searchHelper(new Conjunction(c, next.v, next.b), control, stats, (depth + 1))).status == SearchStatus.Success) {
			return result;
		}
		if (result.status == SearchStatus.Timeout) {
			return result;
		}
		if ((result.status == SearchStatus.Failure) && (result.getAssignment() == null) && (result.getConflictInducedClause() == null)) {
			return result;
		}
		if (control.backjump()) {
		 	if (result.getConflictInducedClause() != null) {
		 		if (!(result.getConflictInducedClause().truthValue(c.assignments))) {
		 			return new SearchResult(result.getConflictInducedClause());
		 		}
		 	}
		}
		if ((result2 = searchHelper(new Conjunction(c, next.v, !next.b), control, stats, (depth + 1))).status == SearchStatus.Success) {
			return result2;
		}
		if (result2.status == SearchStatus.Timeout) {
			return result2;
		}
		if ((result2.status == SearchStatus.Failure) && (result2.getAssignment() == null) && (result2.getConflictInducedClause() == null)) {
			return result2;
		}
		if (control.backjump()) {
			/*if (result2.getConflictInducedClause() != null) {
		 		if (!(result2.getConflictInducedClause().containsLiteral(next.v))) {
		 			return result2;
		 		}
		 	}*/
			if ((result.getConflictInducedClause() != null) && (result2.getConflictInducedClause() == null)) {
				return result;
			}
			if ((result.getConflictInducedClause() == null) && (result2.getConflictInducedClause() != null)) {
				return result2;
			...
			if ((result.getConflictInducedClause() != null) && (result2.getConflictInducedClause() != null)) {
				if (result.getConflictInducedClause().containsLiteral(next.v) && result2.getConflictInducedClause().containsLiteral(next.v)) {
					Disjunction cause = Disjunction.resolve(result.getConflictInducedClause(), result2.getConflictInducedClause());
					return new SearchResult(cause);
				}
			}
		}
	    return result2;
...
//		String o1 = doSearchWithTimeLimit("./Examples/10L-25V-10C.dimacs", onlyUPAndPLSearch, 15000L);
//		System.err.println(o1);
	    String o2 = doSearchWithTimeLimit("./Examples/10L-25V-25C.dimacs", onlyUPAndPLSearch, 15000L);
	    System.err.println(o2);
//	    String o3 = doSearchWithTimeLimit("./Examples/10L-30V-40C.dimacs", onlyUPAndPLSearch, 15000L);
//	    System.err.println(o3);
//	    String o4 = doSearchWithTimeLimit("./Examples/20L-20V-10C.dimacs", onlyUPAndPLSearch, 15000L);
//	    System.err.println(o4);
//	    String o5 = doSearchWithTimeLimit("./Examples/20L-20V-20C.dimacs", onlyUPAndPLSearch, 15000L);
//	    System.err.println(o5);
//	    String o6 = doSearchWithTimeLimit("./Examples/20L-30V-40C.dimacs", onlyUPAndPLSearch, 15000L);
//	    System.err.println(o6);
//	    String o7 = doSearchWithTimeLimit("./Examples/5L-10V-25C.dimacs", onlyUPAndPLSearch, 15000L);
//	    System.err.println(o7);
//	    String o8 = doSearchWithTimeLimit("./Examples/5L-25V-10C.dimacs", onlyUPAndPLSearch, 15000L);
//	    System.err.println(o8);
//	    String o9 = doSearchWithTimeLimit("./Examples/5L-25V-25C.dimacs", onlyUPAndPLSearch, 15000L);
//	    System.err.println(o9);
//	    String o10 = doSearchWithTimeLimit("./Examples/5L-5V-5C.dimacs", onlyUPAndPLSearch, 15000L);
